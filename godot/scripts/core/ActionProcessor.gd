...
func _do_create_meld(state: GameState, player: int, action: Dictionary) -> Dictionary:
	...
	# add meld to table using richer card-entry format
	var card_entries: Array = []
	for cid_any in ordered_card_ids:
		card_entries.append(state.make_meld_card(String(cid_any), player))

	var meld: Dictionary = state.make_meld(meld_kind, card_entries)

	# keep extra metadata for rules/scoring/debugging compatibility
	meld["owner"] = player
	meld["contrib"] = {}
	for cid_any in ordered_card_ids:
		meld["contrib"][String(cid_any)] = player
...
func _do_layoff(state: GameState, player: int, action: Dictionary) -> Dictionary:
	...
	var meld_index := _find_meld_index_by_id(state, meld_id)
	if meld_index == -1:
	...
	if mtype == MELD_SET:
		...
		# validate via MeldRules.can_extend_run_end() using a rules-friendly legacy shape
		var check = MeldRules.can_extend_run_end(_meld_for_rules(meld), card_id, end, registry, rules.allow_wrap_runs)
		if not bool(check.get("ok", false)):
			out.reason = String(check.get("reason", "RUN_LAYOFF_INVALID"))
			return out

		# apply extension to richer card-entry structure
		if end == END_LEFT:
	...
	out.events.append({
		"type": "LAYOFF",
		"player": player,
		"meld_id": int(meld.get("id", meld_id)),
		"card_id": card_id
	})
	return out
...
func _can_layoff_target_to_any_meld(state: GameState, target_card_id: String) -> bool:
	...
	for meld_any in state.melds:
		...
		if mtype == MELD_SET:
			...
			var check_l = MeldRules.can_extend_run_end(rules_meld, target_card_id, END_LEFT, registry, rules.allow_wrap_runs)
			if bool(check_l.get("ok", false)):
				return true
			var check_r = MeldRules.can_extend_run_end(rules_meld, target_card_id, END_RIGHT, registry, rules.allow_wrap_runs)
...
# -------------------------
# Meld shape compatibility helpers
# -------------------------
...
func _meld_card_ids(meld: Dictionary) -> Array:
	...
	for raw_card in Array(meld.get("cards", [])):
		if raw_card is Dictionary:
			out.append(String(raw_card.get("card_id", raw_card.get("id", ""))))
		else:
			out.append(String(raw_card))
	return out
...
func _find_meld_index_by_id(state: GameState, meld_id: int) -> int:
	# Prefer stable public meld id.
	for i in range(state.melds.size()):
		var meld_any = state.melds[i]
		if meld_any is Dictionary and int(meld_any.get("id", -999999)) == meld_id:
			return i

	# Compatibility fallback: older code/tests may still pass array index.
	if meld_id >= 0 and meld_id < state.melds.size():
		return meld_id
