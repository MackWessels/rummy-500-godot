...
func _ready() -> void:
	...
	_test_wrap_runs_toggle_blocks_k_a_2()
	_test_wrap_runs_toggle_still_allows_a23_and_qka()
	_test_no_wrap_blocks_extending_qka_with_2()
	_test_no_wrap_blocks_extending_a23_with_k()
...
func _apply(ap: ActionProcessor, st: GameState, reg: CardRegistry, rules: RulesConfig, p: int, action: Dictionary, expect_ok: bool, label: String) -> Dictionary:
	...
	if not expect_ok:
		...
		var v := Invariants.validate(st, reg, rules)
...
func _test_wrap_runs_toggle_blocks_k_a_2() -> void:
	...
	_eq(bool(res.get("ok", true)), false, "K-A-2 rejected when allow_wrap_runs=false")
	_eq(String(res.get("reason","")), "WRAP_RUNS_DISABLED", "reason is WRAP_RUNS_DISABLED")

func _test_wrap_runs_toggle_still_allows_a23_and_qka() -> void:
	print("\n--- test_wrap_runs_toggle_still_allows_a23_and_qka ---")
	...
	# A-2-3 valid
	var st1 := GameState.new()
	st1.init_for_players(2)
	st1.turn_player = 0
	st1.phase = "PLAY"
	...
	st1.hands[0] = [aS, s2, s3]

	_apply(ap, st1, reg, rules, 0, {"type":"CREATE_MELD", "meld_kind":"RUN", "card_ids":[aS, s2, s3]}, true,
		"allow_wrap_runs=false still allows A-2-3")

	# Q-K-A valid
	var st2 := GameState.new()
	st2.init_for_players(2)
	st2.turn_player = 0
	st2.phase = "PLAY"
	...
	st2.hands[0] = [qH, kH, aH]

	_apply(ap, st2, reg, rules, 0, {"type":"CREATE_MELD", "meld_kind":"RUN", "card_ids":[qH, kH, aH]}, true,
		"allow_wrap_runs=false still allows Q-K-A")

func _test_no_wrap_blocks_extending_qka_with_2() -> void:
	print("\n--- test_no_wrap_blocks_extending_qka_with_2 ---")
	...
	var kH := _pick(reg, 0, "H", 13)
	var aH := _pick(reg, 0, "H", 1)
	var h2 := _pick(reg, 0, "H", 2)

	st.hands[0] = [qH, kH, aH, h2]

	_apply(ap, st, reg, rules, 0, {"type":"CREATE_MELD", "meld_kind":"RUN", "card_ids":[qH, kH, aH]}, true,
		"create Q-K-A hearts")

	var res := _apply(ap, st, reg, rules, 0, {"type":"LAYOFF", "meld_id": 0, "card_id": h2, "end":"RIGHT"}, false,
		"no-wrap blocks extending Q-K-A with 2H")
	_eq(String(res.get("reason", "")), "WRAP_RUNS_DISABLED", "reason is WRAP_RUNS_DISABLED")

func _test_no_wrap_blocks_extending_a23_with_k() -> void:
	print("\n--- test_no_wrap_blocks_extending_a23_with_k ---")
	...
	st.hands[0] = [aS, s2, s3, kS]

	_apply(ap, st, reg, rules, 0, {"type":"CREATE_MELD", "meld_kind":"RUN", "card_ids":[aS, s2, s3]}, true,
		"create A-2-3 spades")

	var res := _apply(ap, st, reg, rules, 0, {"type":"LAYOFF", "meld_id": 0, "card_id": kS, "end":"LEFT"}, false,
		"no-wrap blocks extending A-2-3 with KS on LEFT")
	_eq(String(res.get("reason", "")), "WRAP_RUNS_DISABLED", "reason is WRAP_RUNS_DISABLED")
...
func _test_full_hand_deterministic() -> void:
	...
	_apply(ap, st, reg, rules, 0, {"type":"DRAW_DISCARD_STACK", "target_card_id": h7}, true, "P0 DRAW_DISCARD_STACK target=H7")
	...
	_apply(ap, st, reg, rules, 0, {"type":"DISCARD", "card_id": jd}, false, "P0 DISCARD blocked while must-play pending")
	...
	_apply(ap, st, reg, rules, 0, {"type":"CREATE_MELD", "meld_kind":"SET", "card_ids":[s7, d7, h7]}, true, "P0 CREATE_MELD SET 7s (includes target)")
	...
	_apply(ap, st, reg, rules, 0, {"type":"DISCARD", "card_id": jd}, true, "P0 DISCARD JD ends turn")
	...
	_apply(ap, st, reg, rules, 1, {"type":"DRAW_STOCK"}, true, "P1 DRAW_STOCK")
	_ok(st.hands[1].has(d6), "P1 has drawn D6")

	_apply(ap, st, reg, rules, 1, {"type":"CREATE_MELD", "meld_kind":"RUN", "card_ids":[aS, s2, s3]}, true, "P1 CREATE_MELD RUN A-2-3 spades")
	_eq(st.melds.size(), 2, "table has 2 melds (set + run)")

	_apply(ap, st, reg, rules, 1, {"type":"DISCARD", "card_id": d9}, true, "P1 DISCARD 9D ends turn")
	_eq(st.turn_player, 0, "turn returns to P0")
	_eq(st.phase, "DRAW", "phase is DRAW for P0")

	# Turn 2 (P0): draw stock (AH), layoff 4S, create Q-K-A, discard last to go out
	_apply(ap, st, reg, rules, 0, {"type":"DRAW_STOCK"}, true, "P0 DRAW_STOCK")
	_ok(st.hands[0].has(ah), "P0 has drawn AH")

	_apply(ap, st, reg, rules, 0, {"type":"LAYOFF", "meld_id": 1, "card_id": s4, "end":"RIGHT"}, true, "P0 LAYOFF 4S onto spade run (RIGHT)")

	_apply(ap, st, reg, rules, 0, {"type":"CREATE_MELD", "meld_kind":"RUN", "card_ids":[qh, kh, ah]}, true, "P0 CREATE_MELD RUN Q-K-A hearts (Ace high)")

	var res_out := _apply(ap, st, reg, rules, 0, {"type":"DISCARD", "card_id": c9}, true, "P0 DISCARD last card -> WENT_OUT")
